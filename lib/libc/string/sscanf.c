...
int vsscanf (const char* str, const char* format, va_list ap)
{
	...
	case FLOAT_CONVERSION:
		*va_arg (ap, float*) = (float)(strtod (conversionSource, NULL));
		break;
	case DOUBLE_CONVERSION: 
		*va_arg (ap, double*) = strtod (conversionSource, NULL);
		break;
	case HEX_CONVERSION:
		*va_arg (ap, unsigned int*) = (unsigned int)(strtol (conversionSource, NULL, 16));
		break;
	case HEX64_CONVERSION:
		*va_arg (ap, long unsigned int*) = strtol (conversionSource, NULL, 16);
		break;
	default:
		break;
	}
